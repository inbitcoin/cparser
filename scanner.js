...
Scanner.prototype.fix_blocks = function (err, callback) {
  ...
  self.get_next_block_to_fix(self.limit_fixer, function (err, raw_block_datas) {
    ...
    if (properties.scanner.start_block &&
      ((first_block - 1) != properties.scanner.start_block) &&
      ((first_block - 1) != properties.scanner.start_block + 1) &&
      first_block != 0) {
  ...
  self.Blocks.findOne({txinserted: true},
    ...
    .exec(function (err, block_data) {
      ...
      if (block_data) {
        ...
        if (properties.scanner.start_block && properties.scanner.start_hash) {
          callback(null, properties.scanner.start_block + 1, properties.scanner.start_hash)
        } else {
